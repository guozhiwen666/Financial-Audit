...
flowchart TD
    ...
    G --> H[解析附件: PDF / PNG / JPG]
    H --> I[汇总单据+明细+解析结果+规则+市场价+供应商]
    I --> J[生成风险分析结果]
...
```mermaid
flowchart TD
    A[用户输入] --> B{会话已有已确认单据类型?}
    B -- 否 --> C[发送 slot_required 询问单据类型<br/>candidate_values=五类单据]
    C --> A
    B -- 是 --> D{会话已有已确认单据编号?}
    D -- 否 --> E[发送 slot_required 询问单据编号]
    E --> A
    D -- 是 --> F{信息歧义或冲突?}
    F -- 是 --> G[展示候选项请求确认]
    G --> A
    ...
    H --> I{查询成功?}
    I -- 否 --> J[发送 error 展示失败环节<br/>允许重试]
    J --> A
    I -- 是 --> K{权限校验通过?}
    K -- 否 --> L[提示无数据权限]
    K -- 是 --> M[创建分析任务并推送 task_status]
    M --> N[逐条推送 risk_finding]
    N --> O[推送 report_ready]
    O --> P[推送 done]
```
...
- 槽位状态需持久化（见 9.3 补充建议），保证刷新页面或跨会话恢复后不重复询问。
...
### 12.3 字段覆盖度核对与补充建议（需确认）

比对 2.7.2 单据字段要求与 2.7.10 数据表清单，以下项目在给定表结构中**无对应落点**，建议补充后再开工，否则需求可验证性不足：
...
| 风险 | 影响 | 应对 |
|---|---|---|
| 12.3 中的字段与数据表缺口未确认 | 需求可验证性不足，返工风险高 | 开工前确认 G01~G11 |
...
本文档单据字段与 `schema/financial_document.py` 保持一致，该模块已定义五类单据、明细行、附件、字段提取与文本位置的数据结构，可直接作为后端模型与数据库表设计的对齐依据。

**附录 B：需求可追溯性**

本文档第 4~16 章逐一对应原需求 2.7.1~2.7.14；第 18 章对应 2.7.15 验收标准；第 19 章对应"示例数据与完整审核演示链路"交付要求。
