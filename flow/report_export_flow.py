...
from schema.tables_document import FinancialDocument  # 单据实体（报告「单据摘要」的事实来源）
...
class ReportExportFlow:
    ...
    def generate(self, task_id: int, document: FinancialDocument, findings: list[RiskFinding],
                 overall_level: RiskLevel, amount_comparison: dict, reviews: list[ManualReview],
                 attachment_count: int = 0) -> ReviewReport:
        """生成风险审核报告：交由 A4 撰写正文、聚合面板并推送 report_ready（PRD 第 15 章）。"""
        # 步骤 1：整体等级与金额核对数据均来自既有结论，本环节只做转交，不重新判定（PRD 20.2 A4）
        # 步骤 2：单据实体与附件数量一并转交，供报告「单据摘要」章节引用真实事实（PRD 15）
        # 步骤 3：A4 内部按第 15 章的八个章节撰写正文，并在成功导出后推送 report_ready 消息
        return self.reporter.run(task_id, document, findings, overall_level,
                                 amount_comparison, reviews, attachment_count)
